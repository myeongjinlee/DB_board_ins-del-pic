...
//글 등록
	...
	$upload_dir = 'uploads/';
	$upload_file = $upload_dir . basename($_FILES['bImage']['name']); //경로 + 파일명
	move_uploaded_file($_FILES['bImage']['tmp_name'], $upload_file);

	
	$sql = 'insert into board_free (b_no, b_title, b_content, b_date, b_hit, b_id, b_password,b_image) 
	values(null, "' . $bTitle . '", "' . $bContent . '", "' . $date . '", 0, "' . $bID . '", 
	password("' . $bPassword . '"),"'.$upload_dir . basename($_FILES['bImage']['name']) . '")';
	$msgState = '등록';
}
